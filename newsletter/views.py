...
def subscribe(request):
    ...
    if not (api_key and server and list_id):
        messages.success(
            request,
            "Inschrijving ontvangen. (Lokaal opgeslagen — Mailchimp nog niet geconfigureerd.)",
        )
    ...
    try:
        ...
        payload = {
            "email_address": email,
            "status": "pending" if double else "subscribed",
        }
        client.lists.add_list_member(list_id, payload)
        messages.success(
            request,
            (
                "Check je inbox om je inschrijving te bevestigen."
                if double
                else "Ingeschreven! Bedankt."
            ),
        )
    except Exception:

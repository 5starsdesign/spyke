...
def home(request):
    featured = MarketListing.objects.filter(is_published=True).order_by("-is_featured","-created_at")[:8]
...
def huur_list(request):
    q = MarketListing.objects.filter(is_published=True, listing_kind=MarketListing.K_HUUR).order_by("-created_at")
    return render(request, "public/huur_list.html", {"items": q})
def koop_list(request):
    q = MarketListing.objects.filter(is_published=True, listing_kind=MarketListing.K_KOOP).order_by("-created_at")

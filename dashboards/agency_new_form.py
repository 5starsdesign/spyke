...
class AgencyNewForm(forms.ModelForm):
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        try:
            from django.forms.widgets import (CheckboxInput,
                                              ClearableFileInput, DateInput,
        ...
        for name, field in self.fields.items():
            ...
            elif w.__class__.__name__ in ("Textarea",):
                w.attrs["class"] = (cls + " form-control").strip()
            elif w.__class__.__name__ in ("Select",):
                w.attrs["class"] = (cls + " form-select").strip()
            elif w.__class__.__name__ in ("CheckboxInput",):
                w.attrs["class"] = (cls + " form-check-input").strip()
            elif w.__class__.__name__ in ("RadioSelect",):
                w.attrs["class"] = (cls + " form-check-input").strip()
            elif w.__class__.__name__ in ("ClearableFileInput",):
                w.attrs["class"] = (cls + " form-control").strip()
    ...
    def __init__(self, *args, **kwargs):
        ...
        for name, field in self.fields.items():
            ...
            try:
                ...
                from django.forms.widgets import (CheckboxInput,
                                                  ClearableFileInput,
                                                  DateInput, EmailInput,
                                                  NumberInput, RadioSelect,
                                                  Select, Textarea, TextInput,
                                                  URLInput)

                if isinstance(
                    w,
                    (TextInput, NumberInput, URLInput, EmailInput, DateInput, Textarea),
                ):
                    w.attrs["class"] = (
                        w.attrs.get("class", "") + " form-control"
                    ).strip()
                elif isinstance(w, Select):
                    w.attrs["class"] = (
                        w.attrs.get("class", "") + " form-select"
                    ).strip()
    ...
    status_active = forms.BooleanField(
        required=False, initial=True, label="Status: actief (uit = concept)"
    )
    ...
    property_type = forms.ChoiceField(
        choices=MarketListing._meta.get_field("property_type").choices,
        label="Type woning",
    )
    build_year = forms.IntegerField(
        required=False, label="Bouwjaar", min_value=1000, max_value=3000
    )
    living_area = forms.IntegerField(label="Woonoppervlakte (m²)", min_value=0)
    lot_area = forms.IntegerField(
        required=False, label="Perceeloppervlakte (m²)", min_value=0
    )
    rooms = forms.IntegerField(label="Aantal kamers", min_value=0)
    bedrooms = forms.IntegerField(
        required=False, label="Aantal slaapkamers", min_value=0
    )
    bathrooms = forms.IntegerField(
        required=False, label="Aantal badkamers", min_value=0
    )
    floors = forms.IntegerField(
        required=False, label="Aantal verdiepingen", min_value=0
    )
    ...
    wheelchair_accessible = forms.BooleanField(
        required=False, label="Rolstoeltoegankelijk"
    )
    ...
    price = forms.DecimalField(
        label="Vraagprijs (€) / Huurprijs (€)", max_digits=10, decimal_places=2
    )
    kk_von = forms.ChoiceField(
        choices=(("KK", "k.k."), ("VON", "v.o.n.")),
        required=False,
        widget=forms.RadioSelect,
        label="Kosten",
    )
    vve_service = forms.DecimalField(
        required=False, max_digits=8, decimal_places=2, label="Servicekosten VvE p/m"
    )
    condition_state = forms.CharField(required=False, label="Onderhoudsstaat")

    service_costs = forms.DecimalField(
        required=False, max_digits=8, decimal_places=2, label="Servicekosten p/m"
    )
    deposit = forms.DecimalField(
        required=False, max_digits=10, decimal_places=2, label="Waarborgsom (€)"
    )
    rental_min = forms.IntegerField(
        required=False, min_value=0, label="Huurtermijn min. (mnd)"
    )
    rental_max = forms.IntegerField(
        required=False, min_value=0, label="Huurtermijn max. (mnd)"
    )
    ...
    suitable_for = forms.CharField(
        required=False, label="Geschikt voor (bv. studenten/gezin/expats)"
    )
    ...
    contact_pref_phone = forms.BooleanField(
        required=False, label="Contact per telefoon"
    )
    viewing_slots = forms.CharField(required=False, label="Bezichtigingsmomenten")

    # Publicatie
    show_exact_address = forms.BooleanField(
        required=False, initial=True, label="Toon exact adres"
    )
    ...
    def clean(self):
        ...
        if data.get("price") in (None, ""):
            self.add_error(
                "price",
                (
                    "Huurprijs is verplicht."
                    if kind == "HUUR"
                    else "Vraagprijs is verplicht."
                ),
            )
        return data
    ...
    def to_address(self):
        ...
        tail = " ".join(
            x
            for x in [
                (self.cleaned_data.get("house_no") or ""),
                (self.cleaned_data.get("house_add") or ""),
            ]
            if x
        ).strip()
        if tail:

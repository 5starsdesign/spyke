...
ENERGY_LABELS = [
    ("A++++","A++++"),("A+++","A+++"),("A++","A++"),("A+","A+"),("A","A"),
    ("B","B"),("C","C"),("D","D"),("E","E"),("F","F"),("G","G"),("ONBEKEND","Onbekend"),
]
FINISH_CHOICES = [("KAAL","Kaal"),("GESTOFFEERD","Gestoffeerd"),("GEMEUBILEERD","Gemeubileerd")]
RENTAL_TERM_CHOICES = [("BEPAALDE_TIJD","Bepaalde tijd"),("ONBEPAALDE_TIJD","Onbepaalde tijd")]
...
class MarketListing(Timestamped):
    ...
    # Basis
    title = models.CharField(max_length=200, blank=True)  # optioneel; adres kan titel vervangen
    ...
    # Vaste kenmerken
    living_area = models.PositiveIntegerField(validators=[MinValueValidator(1)], help_text="m²")
    lot_area = models.PositiveIntegerField(blank=True, null=True, help_text="Perceel m²")
    ...
    energy_label = models.CharField(max_length=10, choices=ENERGY_LABELS, default="ONBEKEND")
    ...
    # HUUR-specifiek
    service_costs = models.DecimalField(max_digits=10, decimal_places=2, blank=True, null=True)
    deposit = models.DecimalField(max_digits=12, decimal_places=2, blank=True, null=True)
    available_from = models.DateField(blank=True, null=True)
    rental_term = models.CharField(max_length=20, choices=RENTAL_TERM_CHOICES, blank=True)
    finish = models.CharField(max_length=20, choices=FINISH_CHOICES, blank=True)
    pets_allowed = models.BooleanField(default=False)
    smoking_allowed = models.BooleanField(default=False)
    suitable_for = models.TextField(blank=True, help_text="CSV: studenten, gezin, expats")
    ...
    def save(self, *args, **kwargs):
        if not self.slug:
            base = f"{(self.title or self.address or 'woning')}-{self.city}".lower().strip().replace(" ", "-")
...
class MarketListingImage(models.Model):
    listing = models.ForeignKey(MarketListing, on_delete=models.CASCADE, related_name="images")
...
class ExchangeOffer(Timestamped):
    member = models.ForeignKey(settings.AUTH_USER_MODEL, on_delete=models.CASCADE, related_name="exchange_offers")
    my_title = models.CharField(max_length=200, blank=True)
    ...
    my_property_type = models.CharField(max_length=20, choices=PROPERTY_TYPES, blank=True)
    my_tenure = models.CharField(max_length=4, choices=TENURE_CHOICES, blank=True, null=True)
    my_price = models.DecimalField(max_digits=10, decimal_places=2, blank=True, null=True)
    my_rent = models.DecimalField(max_digits=10, decimal_places=2, blank=True, null=True)
    my_description = models.TextField(blank=True)
    my_living_area = models.PositiveIntegerField(blank=True, null=True, validators=[_MV(1)])
    my_rooms = models.PositiveIntegerField(blank=True, null=True, validators=[_MV(1)])
    my_bedrooms = models.PositiveIntegerField(blank=True, null=True, validators=[_MV(0)])
    ...
    my_floorplan = models.ImageField(upload_to="exchange_floorplans/", blank=True, null=True)
    want_city = models.CharField(max_length=120, blank=True)
    want_property_type = models.CharField(max_length=20, choices=PROPERTY_TYPES, blank=True)
    want_min_area = models.PositiveIntegerField(blank=True, null=True, validators=[_MV(1)])
    want_min_rooms = models.PositiveIntegerField(blank=True, null=True, validators=[_MV(1)])
    ...
    def clean(self):
        if self.is_published:
            ...
            errs = {}
            for f in ["my_title","my_city","my_property_type","my_living_area","my_rooms"]:
...
class ExchangeOfferImage(models.Model):
    offer = models.ForeignKey(ExchangeOffer, on_delete=models.CASCADE, related_name="images")
    image = models.ImageField(upload_to="exchange_images/")

class Wish(models.Model):
    user = models.OneToOneField(settings.AUTH_USER_MODEL, on_delete=models.CASCADE, related_name="wish")
    type = models.CharField(max_length=20, choices=PROPERTY_TYPES, blank=True, null=True)
    price = models.DecimalField(max_digits=10, decimal_places=2, blank=True, null=True, help_text="Max. huurprijs (€)")
    rooms = models.PositiveIntegerField(blank=True, null=True)
    bedrooms = models.PositiveIntegerField(blank=True, null=True)
    size = models.PositiveIntegerField(blank=True, null=True, help_text="Min. woonoppervlakte (m²)")
    province = models.CharField(max_length=100, blank=True, null=True)
    city = models.CharField(max_length=120, blank=True, null=True, help_text="Gewenste plaats(en), komma-gescheiden")
    want_garden = models.BooleanField(default=False)
    want_balcony = models.BooleanField(default=False)
    want_terrace = models.BooleanField(default=False)
    want_storage = models.BooleanField(default=False)
    want_shed = models.BooleanField(default=False)
    want_garage = models.BooleanField(default=False)
    want_near_shops = models.BooleanField(default=False)
    want_near_schools = models.BooleanField(default=False)
    want_near_ov = models.BooleanField(default=False)
    want_ground_floor = models.BooleanField(default=False)
    want_lift = models.BooleanField(default=False)
    want_wheelchair = models.BooleanField(default=False)
    want_pets_allowed = models.BooleanField(default=False)
